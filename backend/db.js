...
const DB_PATH = path.join(__dirname, 'haraka.sqlite')
...
db.serialize(() => {
  ...
  // Seed default admin — password is: Admin@Haraka2025
  db.get('SELECT id FROM admin_users LIMIT 1', (err, row) => {
    if (!row) {
      db.run(
        `INSERT INTO admin_users (username, password) VALUES (?,?)`,
        ['admin', '$2b$12$F/eieoUOGSabcoytguadyOTCIRnC.PqG/ExSz7NU.1lMjWcRGGgMu'],
        () => console.log('  ◆  Default admin created — username: admin  password: Admin@Haraka2025')
      )
    }
  })
...
db.getExpiryAlerts = (daysAhead = 30, callback) => {
  ...
  const done = () => { pending--; if (pending === 0) callback(alerts) }
  ...
  db.all(`SELECT id, registration, make, model, motExpiry, taxExpiry, insuranceExpiry, pcoPlateExpiry
          FROM vehicles WHERE status = 'active'`, [], (err, vehicles) => {
    if (!err) {
      vehicles.forEach(v => {
